            ...
            else
            {
                ...
                foreach (Gate g in gatesList)
                {
                    if (g.IsMouseOn(startX, startY))
                    {
                        ...
                        Compound c = Compound.RecursiveReturnCompound(current, gatesList);
                        ...
                        if (newCompound != null)
                        {
                            Compound cg = (Compound)newCompound;
                            cg.AddGate(current);                           
        ...
        private void toolStripButtonCopy_Click(object sender, EventArgs e)
        {
            if (current != null)
            {
                ...
                if (current is Compound)
                {
                    ...
                    Compound.WireCloning(Clone, Original, ref wiresList);
                }
                current.Selected = false;
                current = null;
            }
...
/*
A new compound gate will be created when ever a compound joins a compound, or a single gate joins a compound, or a single gate joins a single. 
There are code in my program to handle each of those cases. The original compound gate is also keepped, so if the program requires,
some piece of code can be written to broken down a wide range compound gate into its subunits(narrower ranged compound gate, or just single gate). 
*/
